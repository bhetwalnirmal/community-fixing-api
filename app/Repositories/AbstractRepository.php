...
abstract class AbstractRepository
{
    ...
    public function create(array $data, array $fields = [], User $user = null): Model
    {
        ...
        return $this->getById($entity->{$this->entity_key}, $fields);
    }
    ...
    public function getById($id, array $fields = [], $user = null): ?Model
    {
        $query = $this->createBaseBuilder($fields, [], $user);
    ...
    public function getListQuery(array $fields = [], array $args = [], User $user = null, array $scopes = [])
    {
        $query = $this->createBaseBuilder($fields, $args, $user);

        if ($scopes) {
            $query
                ->where(function ($query) use ($scopes, $user) {
                    foreach ($scopes as $scope) {
                        $query
                            ->orWhere(function ($q) use ($scope, $user) {
                                $q->{$scope}($user);
                            });
                    }
                });
    ...
    public function update(Model $entity, array $data = [], array $fields = [], User $user = null): ?Model
    {
        $entity->update($data);
    ...
    protected function applyFilter($query, array $filter, &$joins, $or = false)
    {
        $key = $filter['key'];
        ...
        if ('null' === $value || '' === $value) {
            ...
            if ($custom_filter_method) {
                ...
                if (in_array($operator, ['in', 'bt'])) {
                    ...
                    call_user_func_array([$query, $method], [
                        $database_field, $clause_operator, $value,
                    ]);
                }
            }
        }
    ...
    protected function applyFilters($query, $filters, &$joins, $or = false)
    {
        $method = $or ? 'orWhere' : 'where';

        if (isset($filters['AND'])) {
            $query->{$method}(function ($query) use ($filters, &$joins) {
     ...
     * @return bool
     */
    protected function checkQueryHasJoinedTable(Builder $query, $table)
    {
    ...
    protected function createBaseBuilder(array $fields = [], array $args = [], $user = null)
    {
